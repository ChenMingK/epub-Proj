...
- nth是scss预设方法，可以从数组中获取指定下标的元素，如上面的nth($moves, $index)表示从$moves数组中取出第$index个元素
- map-get是scss预设方法，可以从Maps结构中取出指定的属性的值，如上面的map-get($item, width)表示从$item这个Maps结构中取出width属性的值
- scss变量作为名称使用时需要加上#{}，如上面的@keyframes #{$keyframesName}

这里贴出官方的map-get的使用方法便于理解，顺便给出<a href="https://www.sass.hk/docs/">官方教程</a>
